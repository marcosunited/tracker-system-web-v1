<html>

<head>
    <title>Compliance Certification</title>
    <style type="text/css">
        body {
            font-size: 19.7px;
            margin: 0px;
        }

        .pos {
            position: absolute;
            z-index: 0;
            left: 0px;
            top: 0px
        }

        * {
            font-family: sans-serif;
        }
    </style>

    <script src="{{ mix('js/dashmix.app.js') }}"></script>
    <script src="{{ mix('js/laravel.app.js') }}"></script>

</head>

<body>
    <nobr>
        <nowrap>
            <div class="pos" id="_0:0" style="top:0">
                <img name="_1170:827" src="http://cloud.unitedlifts.com.au:8070/image/custom-reports/compliance.jpg" height="980" width="680" border="0" usemap="#Map">
            </div>
            <div class="pos" id="_98:136" style="top:84;left:10">
                <span id="_21.7" style="font-weight:bold; font-size:21.7px; color:#003490">
                    COMPLIANCE CERTIFICATION</span>
            </div>
            <div class="pos" id="_333:163" style="top:110;left:186">
                <span id="_14.9" style="font-weight:bold; font-size:14.9px; color:#e17900">
                    Compliance Certification </span>
            </div>
            <div class="pos" id="_76:188" style="top:130;left:14">
                <span id="_14.0" style="font-weight:bold; font-size:14.0px; color:#000000">
                    Contract Name: <span style="font-weight:normal"> Whole of Government Facilities </span></span>
            </div>
            <div class="pos" id="_411:188" style="top:130;left:250">
                <span id="_14.0" style="font-weight:bold; font-size:14.0px; color:#000000">
                    Contract No.: <span style="font-weight:normal"> {{$maintenance->lifts->location == 'North' ? '7061005' : '7061007' }}</span></span>
            </div>
            <div class="pos" id="_76:207" style="top:142;left:14">
                <span id="_14.0" style=" font-size:14.0px; color:#000000">
                    Management Services Contract</span>
            </div>
            <div class="pos" id="_76:227" style="top:162;left:14">
                <span id="_14.0" style="font-weight:bold; font-size:14.0px; color:#000000">
                    Facility / Site:<span style="font-weight:normal"> {{$maintenance->job_name}} </span></span>
            </div>
            <div class="pos" id="_411:227" style="top:162;left:250">
                <span id="_14.0" style="font-weight:bold; font-size:14.0px; color:#000000">
                    Category of Work: 2.2.10 - <span style="font-weight:normal"> Passenger Lifts</span></span>
            </div>
            <div class="pos" id="_76:252" style="top:185;left:14">
                <span id="_14.0" style="font-weight:bold; font-size:14.0px; color:#000000">
                    Contractor:<span style="font-weight:normal"> United Lift Services </span> </span>
            </div>
            <div class="pos" id="_411:253" style="top:185;left:250">
                <span id="_14.0" style="font-weight:bold; font-size:14.0px; color:#000000">
                    Competent Person: <span style="font-weight:normal">{{$maintenance->techs->technician_name}}</span></span>
            </div>
            <div class="pos" id="_76:285" style="top:208;left:14">
                <span id="_15.2" style="font-weight:bold; font-size:15.2px; color:#e17900">
                    Compliance Description:</span>
            </div>
            <div class="pos" id="_76:317" style="top:232;left:14">
                <span id="_15.3" style=" font-size:15.3px; color:#000000">
                    This certificate is for the above mentioned Facility to certify that the following essential services </span>
            </div>
            <div class="pos" id="_76:348" style="top:248;left:14">
                <span id="_15.3" style=" font-size:15.3px; color:#000000">
                    have been tested in accordance with the following standards; AS 1735.1 - Lifts, escalators and</span>
            </div>
            <div class="pos" id="_76:378" style="top:264;left:14">
                <span id="_15.3" style=" font-size:15.3px; color:#000000">moving
                    <span id="_15.3" style="font-style:italic; font-size:15.3px; color:#000000">
                        walks Passenger and goods lifts </span></span>
            </div>
            <div class="pos" id="_76:565" style="top:385;left:14;right:20px">
                <span id="_15.3" style=" font-size:15.3px;">
                    {{$maintenance->techs->technician_name}}, who is qualified to undertake the relevant assessments, undertook the</span>
            </div>
            <div class="pos" id="_76:586" style="top:400;left:14">
                <span id="_15.3" style=" font-size:15.3px; color:#000000">
                    testing and commissioning of these systems on {{ \Carbon\Carbon::parse($maintenance->maintenance_date)->format('d/m/Y') }}</span>
            </div>
            <div class="pos" id="_76:643" style="top:450;left:14">
                <span id="_15.3" style=" font-size:15.3px; color:#000000">
                    Yours Sincerely<span id="_12.7" style=" font-size:12.7px"> , {{$maintenance->techs->technician_name}}</span></span>
            </div>
            <div class="pos" id="_76:685" style="top:530;left:14">
                <span id="_15.1" style="font-weight:bold; font-size:15.1px; color:#e17900">
                    Sub-Contractors Details:</span>
            </div>
            <div class="pos" id="_14:685" style="top:660;left:18">
                <img src="http://cloud.unitedlifts.com.au:8070/image/logo.png" style="width: 205px; height:49px">
            </div>

            <div class="pos" id="_523:1110" style="top:640;left:180">
                <span id="_11.0" style=" font-size:11.0px; color:#888888">
                    United Lift Services</span>
            </div>
            <div class="pos" id="_523:1110" style="top:650;left:180">
                <span id="_11.0" style=" font-size:11.0px; color:#888888">
                    ABN: 81 082 447 658</span>
            </div>
            <div class="pos" id="_523:1110" style="top:660;left:180">
                <span id="_11.0" style=" font-size:11.0px; color:#888888">
                    License number 60398S</span>
            </div>
            <div class="pos" id="_523:1110" style="top:670;left:180">
                <span id="_11.0" style=" font-size:11.0px; color:#888888">
                    Unit 6, 38 Raymond Avenue Matraville</span>
            </div>
            <div class="pos" id="_523:1110" style="top:680;left:180">
                <span id="_11.0" style=" font-size:11.0px; color:#888888">
                    1300 161 740</span>
            </div>
            <div class="pos" id="_523:1110" style="top:690;left:180">
                <span id="_11.0" style=" font-size:11.0px; color:#888888;text-decoration:underline">
                    sydney@unitedlifts.com.au</span>
            </div>

            <div class="pos" id="_98:1110" style="top:760;left:18">
                <span id="_11.0" style=" font-size:11.0px; color:#000000">
                    Page <span style="font-weight:bold"> 1</span> of <span style="font-weight:bold"> 1</span></span>
            </div>
            <div class="pos" id="_523:1110" style="top:760;left:346">
                <span id="_11.0" style=" font-size:11.0px; color:#000000">
                    OPS-CC-2.2.10-AMC- Compliance Certificate</span>
            </div>
            <div class="pos" id="_598:1123" style="top:772;left:402">
                <span id="_11.0" style=" font-size:11.0px; color:#000000">
                    Document Control by WHSEQ</span>
            </div>
            <div class="pos" id="_337:1136" style="top:772;left:200">
                <span id="_11.0" style=" font-size:11.0px; color:#000000">
                    Facilites First Australia Pty Ltd</span>
            </div>
            <div class="pos" id="_639:1136" style="top:784;left:432">
                <span id="_11.0" style=" font-size:11.0px; color:#000000">
                    Issue Date: May 2020</span>
            </div>
        </nowrap>
    </nobr>
</body>

</html>