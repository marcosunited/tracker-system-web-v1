...
class MaintenanceController extends Controller
{
    ...
    {
        $selectlifts = Lift::find($request->message);
        Session::put('selectlifts',$selectlifts);

        if ($selectlifts->lift_type == 'L') {
    ...
    {
        if( request('yearmonth') == null) // form in create mode
        {
            $year = date("Y");
            $active_month = isset($request['active_month'])?$request['active_month']:1;
            //$active_month = trim( substr( request('active_month') ,5 ,2 ));
            if( strlen($active_month) == 1 ) $active_month = "0" . $active_month;
            $year_month = $year . $active_month;                
        }
        ...
        if (isset($request['task_month1'])) { foreach ($request['task_month1'] as $one_task) { $task['jan'][] = $one_task;}}
        if (isset($request['task_month2'])) {foreach ($request['task_month2'] as $one_task) { $task['feb'][] = $one_task;}}
        if (isset($request['task_month3'])) {foreach ($request['task_month3'] as $one_task) { $task['mar'][] = $one_task;}}
        if (isset($request['task_month4'])) {foreach ($request['task_month4'] as $one_task) { $task['apr'][] = $one_task;}}
        if (isset($request['task_month5'])) {foreach ($request['task_month5'] as $one_task) { $task['may'][] = $one_task;}}
        if (isset($request['task_month6'])) {foreach ($request['task_month6'] as $one_task) { $task['jun'][] = $one_task;}}
        if (isset($request['task_month7'])) {foreach ($request['task_month7'] as $one_task) { $task['jul'][] = $one_task;}}
        if (isset($request['task_month8'])) {foreach ($request['task_month8'] as $one_task) { $task['aug'][] = $one_task;}}
        if (isset($request['task_month9'])) {foreach ($request['task_month9'] as $one_task) {$task['sep'][] = $one_task; }}
        if (isset($request['task_month10'])) {foreach ($request['task_month10'] as $one_task) { $task['oct'][] = $one_task;}}
        if (isset($request['task_month11'])) {foreach ($request['task_month11'] as $one_task) { $task['nov'][] = $one_task;}}                                                                               
        if (isset($request['task_month12'])) {foreach ($request['task_month12'] as $one_task) {$task['dec'][] = $one_task;}}      
        ...
        //$maintenance->lifts()->sync($request->lift_id,false);
    ...
    public function techdetails(MaintenanceN $maintenance)
    {
        ...
        return view('maintenance.showMaintenanceTechDetails',compact('maintenance','tasks','selected_task_month','month_label',
    ...
    public function maintenancejob(MaintenanceN $maintenance)
    {
        ...
        return view('maintenance.maintenanceJob',compact
        ('maintenance','job','selectedRound','rounds','selectedFrequency','frequency','selectedAgent','agents','selectedContract','contract','status','selectedStatus'));
    }
    ...
    public function print($id)
    {
        ...
        return view('maintenance.printmaintenance',compact('maintenance','month_label','tasks','logo','lift'));
    }
